...
<div class="caveat">
📍 <b>위치 정확도 안내:</b> 지도에 표시된 안전등급(D~A)과 위험점수는 `risk_grade_model.py`가
48개 상권의 실측 데이터로 계산한 결과 그대로다.<br>
다만 마커의 정확한 좌표(위경도)는 각 상권이 위치한 동/구의 중심점으로 표시한 근사치이며,<br>
실제 건물의 정밀 주소 좌표는 아니다.(예: 동대문상가 A~D동은 실제로 같은 복합건물 내 동(wing) 구분이라 근사치로도 위치가 크게 다르지 않지만, 일부 인접 건물은 정밀도가 떨어질 수 있다.)<br>
