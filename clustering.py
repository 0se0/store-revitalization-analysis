...
<div class="source">※ 분석 방법: K-Means 클러스터링 (k=3, 6개 변수 표준화, k-means++ 초기화, random_state=42) | 데이터: 한국부동산원 상업용부동산 임대동향조사 (2026년 1분기 실제 공표 수치) | 세종은 오피스 임대동향 조사 미실시 지역으로 K-means 학습(16개 시도)에서는 제외했고, 실측된 상가 공실률·임대료 4개 변수만으로 최근접 중심점(부분거리 방식)을 찾아 군집을 배정했다. 오피스 값은 채우지 않았으며, 클러스터 평균에도 세종의 오피스 수치는 포함하지 않았다.</div>
